...
<!-- <select class="form-control" *ngIf="dropdownVisible" [class.empty]="emptyList" multiple='multiple'>
<option *ngIf="isLoading && loadingTemplate" class="loading" [innerHTML]="loadingTemplate"></option>
<option *ngIf="isLoading && !loadingTemplate" class="loading">{{loadingText}}</option>
<option *ngIf="minCharsEntered && !isLoading && !filteredList.length" (mousedown)="selectOne('')"  class="no-match-found">{{noMatchFoundText || 'No Result Found'}}</option>
<option *ngIf="headerItemTemplate && filteredList.length" class="header-item" [innerHTML]="headerItemTemplate"></option>
<option *ngIf="blankOptionText && filteredList.length" (mousedown)="selectOne('')" class="blank-item">{{blankOptionText}}></option>
<option  class="item" *ngFor="let item of filteredList; let i=index; trackBy: trackByIndex" (mousedown)="selectOne(item)" [ngClass]="{selected: i === itemIndex}" [innerHtml]="autoComplete.getFormattedListItem(item)"></option>
</select> -->
